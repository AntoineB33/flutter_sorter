...
class _SpreadsheetPageState extends State<SpreadsheetPage> {
  ...
  final _collection = 'spreadsheet_data';
  ...
  Future<void> _loadFromFirestore() async {
    final snapshot = await _firestore.collection(_collection).orderBy('row').get();

    if (snapshot.docs.isEmpty) {
      // When no data in Firestore, just initialize visible rows
      setState(() {
        _dataSource = SpreadsheetDataSource(rowsCount: _minRowCount, colsCount: _columnCount);
      });
      return;
    }

    final List<List<dynamic>> gridData = [];
    for (var doc in snapshot.docs) {
      final data = doc.data();
      final cells = List.generate(_columnCount, (i) => data['col_${i + 1}'] ?? '');
      gridData.add(cells);
    }

    setState(() {
      _dataSource.loadFromData(gridData);
    });

    _adjustRowCount(); // ensure visible space is filled
  }

  void _listenToFirestore() {
    _firestore.collection(_collection).snapshots().listen((snapshot) {
      if (_isSyncing) return; // avoid echoing back our own updates
  ...
  void _adjustRowCount() {
    final lastUsedRow = _dataSource.getLastNonEmptyRowIndex() + 1;

    // Always keep at least `_minRowCount` rows visible
    final targetRows = math.max(lastUsedRow, _minRowCount);

    setState(() {
      _dataSource.trimOrExtendTo(targetRows);
    });
  }
  ...
  void initState() {
    ...
    _loadFromFirestore().then((_) => _adjustRowCount());
    _listenToFirestore();
  }
...
class SpreadsheetDataSource extends DataGridSource {
  ...
  void loadFromData(List<List<dynamic>> gridData) {
    _rows.clear();
    for (int i = 0; i < gridData.length; i++) {
      final rowCells = [
        DataGridCell(columnName: 'RowHeader', value: i + 1),
        ...List.generate(
          colsCount,
          (j) => DataGridCell(columnName: columnLetter(j), value: gridData[i][j]),
        )
      ];
      _rows.add(DataGridRow(cells: rowCells));
    }
    notifyListeners();
  }
  ...
  bool setCellValue(DataGridRow row, String columnName, dynamic value) {
    ...
    notifyListeners();
    _syncToFirestore(rowIndex);
    return true;
  }

  Future<void> _syncToFirestore(int rowIndex) async {
    try {
      ...
      for (int i = 0; i < colsCount; i++) {
        final colName = columnLetter(i);
        final cell = row.getCells().firstWhere((c) => c.columnName == colName);
        data['col_${i + 1}'] = cell.value;
      }

      await firestore.collection('spreadsheet_data').doc('row_${rowIndex + 1}').set({
        'row': rowIndex,
        ...data,
  ...
  void trimOrExtendTo(int targetCount) {
    if (_rows.length == targetCount) return;

    if (_rows.length > targetCount) {
      _rows.removeRange(targetCount, _rows.length);
    } else {
      while (_rows.length < targetCount) {
        addRow();
      }
    }
    notifyListeners();
