...
import '../../domain/constants/spreadsheet_constants.dart';
...
class SideMenuTreeBuilder {
  ...
  final NodesUsecase nodesUsecase;
  
  // Specific Analysis Data required by populate methods
  final List<List<HashSet<Attribute>>> tableToAtt;
  final Map<Attribute, Map<int, List<int>>> attToRefFromAttColToCol;
  ...
  int get colCount => table.isEmpty ? 0 : table[0].length;
  ...
  void populateCellNode(NodeStruct node, bool populateChildren) {
    ...
    if (node.message == null) {
      if (node.instruction == SpreadsheetConstants.selectionMsg) {
        node.message =
            '${nodesUsecase.getColumnLabel(colId)}$rowId selected: ${table[rowId][colId]}';
      } else {
        node.message = '${nodesUsecase.getColumnLabel(colId)}$rowId: ${table[rowId][colId]}';
      }
    }
    if (!populateChildren) {
      return;
    ...
    if (node.defaultOnTap) {
      node.onTap = (n) {
        ...
        for (int i = 0; i < cells.length; i++) {
          final child = cells[i];
          if (selectionStart.x == child.rowId && selectionStart.y == child.colId) {
            found = i;
            break;
          }
        }
        if (found == -1) {
          onSelectCell(cells[0].rowId, 0);
        } else {
          onSelectCell(
  ...
  void populateColumnNode(NodeStruct node, bool populateChildren) {
    node.message ??=
        'Column ${nodesUsecase.getColumnLabel(node.colId!)} "${table[0][node.colId!]}"';
    if (!populateChildren) {
      return;
    }
  ...
  void populateNodeDefault(NodeStruct node, bool populateChildren) {
    ...
    if (node.defaultOnTap) {
      ...
      node.onTap = (n) {
        ...
        if (found == -1) {
          onSelectCell(node.cellsToSelect![0].rowId, 0);
        } else {
          onSelectCell(
      ...
      case SpreadsheetConstants.cycleDetected:
        node.onTap = (n) {
          ...
          for (int i = 0; i < n.newChildren!.length; i++) {
            ...
            if (selectionStart.x == child.rowId) {
              found = i;
              break;
            }
          }
          if (found == -1) {
            onSelectCell(n.newChildren![0].rowId!, 0);
          } else {
            onSelectCell(
