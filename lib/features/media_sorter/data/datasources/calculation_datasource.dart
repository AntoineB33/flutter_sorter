...
class CalculationDatasource {
  ...
  static List<int> getValidStarters(
    int n,
    int id,
    List<List<int>> validIndexesMap,
  ) {
    List<int> candidates = [];
    ...
    for (int val in candidates) {
      ...
      List<List<int>> validIndexesMapIfVal = validIndexesMap
          .map((lst) => List<int>.from(lst))
          .toList();
  ...
  static List<List<int>>? getValidPlaces(
    ...
    List<List<int>> newValidAreas = validAreas
    ...
    if (rules[i] != null) {
      rules[i]!.forEach((rel, ruleList) {
        if (validAreas[rel].length > 1 ||
            (validAreas[rel].isNotEmpty && validAreas[rel][0] > id)) {
          ...
          for (int vAId = 0; vAId < validAreas[rel].length; vAId++) {
            ...
            for (SortingRule rule in ruleList) {
              if (rule.minVal <= -(index - id) &&
                  -(index - id) <= rule.maxVal) {
                nowValid[vAId] = true;
              }
            }
          }
          ...
          newValidAreas[rel] =
              filtered; // Update newValidAreas, not validAreas directly
        }
      });
    }
    ...
    for (int rowId in rules.keys) {
      ...
      if (validAreas[rowId].length > 1 ||
          (validAreas[rowId].isNotEmpty && validAreas[rowId][0] > id)) {
  ...
  static void solveSorting(
    ...
    while (id >= 0) {
      ...
      for (int c = 0; c < possibleIntsById[id].length; c++) {
        ...
        if (newValidPlaces != null) {
          // Assuming empty list is valid, null is failure (if logic dictates)
